...
$sql.="  from jadwal WHERE kategori_kelas='SD'";
$query=mysqli_query($conn, $sql) or die("ajax_grid_d_siswa_sd.php: get d_siswa_sd");
...
if( !empty($requestData['search']['value']) ) {
	...
	$query=mysqli_query($conn, $sql) or die("ajax_grid_d_siswa_sd.php: get d_siswa_sd"); // again run query with limit
	
} else {	

	$sql = "select kd_jadwal, nama_pelajaran, nama_guru, kategori_kelas, hari, tanggal, jam_mulai, jam_selesai, nama_ruang, ";
$sql.="if(tanggal = curdate(),  'benar', 'salah') as hariini,if(tanggal >= curdate(), 'benar', 'salah') as besok, if(jam_selesai >= curtime(), 'benar', 'salah') as jam from jadwal WHERE kategori_kelas='SD'";
	$query=mysqli_query($conn, $sql) or die("ajax_grid_d_siswa_sd.php: get d_siswa_sd");   
